...
def sample_frames(flag, num_frames, dataset_name, extra_id = 0):
    '''
    ...
    for i in range(length):
        ...
        photo_id = all_label_json[i]['photo_id'] + extra_id
        ...
        if (frame_prefix != saved_frame_prefix or i == length - 1):
            ...
            for j in range(num_frames):
                ...
                dict['photo_path'] = saved_frame_prefix + '/' + str(
                    single_video_frame_list[(6 + j * frame_interval) % single_video_frame_num]) + '.jpg'
                dict['photo_label'] = single_video_label
                dict['photo_id'] = single_id_label
...
def sample_frames_nolabel(num_frames, dataset_name):
    '''
        from every video (frames) to sample num_frames to test
        return: the choosen frames' path and label
    '''
    ...
    label_path = root_path + '/no_label.json'
    save_label_path = root_path + '/choose_no_label.json'
    ...
    for i in range(length):
        photo_path = all_label_json[i]['photo_path']
        photo_label = all_label_json[i]['photo_label']
        photo_id = all_label_json[i]['photo_id']
        frame_prefix = '/'.join(photo_path.split('/')[:-1])
        # the last frame
        if (i == length - 1):
            photo_frame = int(photo_path.split('/')[-1].split('.')[0])
        ...
        single_video_label = photo_label
        single_id_label = photo_id
        # print("Total video number(no_label): ", video_number, dataset_name)
    json.dump(final_json, f_sample, indent=4)
    f_sample.close()
...
def save_checkpoint(save_list, is_best, model_g, model_cls, model_dec, gpus, checkpoint_path, best_model_path, filename='_checkpoint.pth.tar'):
    epoch = save_list[0]
    ...
    if(len(gpus) > 1):
        old_state_dict_g = model_g.state_dict()
        old_state_dict_cls = model_cls.state_dict()
        old_state_dict_dec = model_dec.state_dict()
        ...
        for k, v in old_state_dict_cls.items():
            flag = k.find('.module.')
            if (flag != -1):
                k = k.replace('.module.', '.')
            new_state_dict_cls[k] = v
        for k, v in old_state_dict_dec.items():
            flag = k.find('.module.')
            if (flag != -1):
                k = k.replace('.module.', '.')
        ...
        state_cls = {
            "epoch": epoch,
            "state_dict_cls": new_state_dict_cls,
            "valid_arg": valid_args,
            "best_model_EER": best_model_HTER,
            "best_model_ACER": best_model_ACER,
            "best_model_ACC": best_model_ACC,
        ...
        state_dec = {
            "epoch": epoch,
            "state_dict_dec": new_state_dict_dec,
            "valid_arg": valid_args,
            "best_model_EER": best_model_HTER,
            "best_model_ACER": best_model_ACER,
            "best_model_ACC": best_model_ACC,
        ...
        state_cls = {
            "epoch": epoch,
            "state_dict_cls": model_cls.state_dict(),
            "valid_arg": valid_args,
            "best_model_EER": best_model_HTER,
            "best_model_ACER": best_model_ACER,
            "best_model_ACC": best_model_ACC,
        ...
        state_dec = {
            "epoch": epoch,
            "state_dict_dec": model_dec.state_dict(),
            "valid_arg": valid_args,
            "best_model_EER": best_model_HTER,
            "best_model_ACER": best_model_ACER,
            "best_model_ACC": best_model_ACC,
    ...
    filepath_g = checkpoint_path + 'G_fas' + filename
    filepath_cls = checkpoint_path + 'C_fas' + filename
    filepath_dec = checkpoint_path + 'Dec_fas' + filename
    torch.save(state_g, filepath_g)
    torch.save(state_cls, filepath_cls)
    torch.save(state_dec, filepath_dec)
    # just save best model
    if is_best:
        shutil.copy(filepath_g, best_model_path + 'model_best_G_fas_' + str(best_model_HTER) + '_' + str(epoch) + '.pth.tar')
        shutil.copy(filepath_cls, best_model_path + 'model_best_Cls_fas_' + str(best_model_HTER) + '_' + str(epoch) + '.pth.tar')
        shutil.copy(filepath_dec, best_model_path + 'model_best_Dec_fas_' + str(best_model_HTER) + '_' + str(epoch) + '.pth.tar')

def save_checkpoint_id(save_list, is_best, model_g, model_d, gpus, checkpoint_path, best_model_path, filename='_checkpoint.pth.tar'):
    epoch = save_list[0]
    valid_args = save_list[1]
    best_model_HTER = round(save_list[2], 5)
    best_model_ACC = save_list[3]
    ...
    if(len(gpus) > 1):
        old_state_dict_g = model_g.state_dict()
        old_state_dict_d = model_d.state_dict()
        from collections import OrderedDict
        new_state_dict_g = OrderedDict()
    ...
    filepath_g = checkpoint_path + 'G_id' + filename
    filepath_d = checkpoint_path + 'Cls_id' + filename
    torch.save(state_g, filepath_g)
    torch.save(state_d, filepath_d)
    # just save best model
    if is_best:
        shutil.copy(filepath_g, best_model_path + 'model_best_G_id_' + str(best_model_HTER) + '_' + str(epoch) + '.pth.tar')
        shutil.copy(filepath_d, best_model_path + 'model_best_Cls_id_' + str(best_model_HTER) + '_' + str(epoch) + '.pth.tar')
